...
const AUDIT_METADATA_ALLOWLIST: Record<AuditActionType, readonly string[]> = {
  ...
  ANAMNESE_VALIDATED_BY_PROFESSIONAL: ["anamneseId", "pacienteId"],
  PDF_VIEWED_BY_PATIENT: ["laudoId", "pacienteId", "pdfType", "profile"],
  PLANO_VALIDATED: [
    "laudoId",
    "pacienteId",
    "professionalValidationConfirmed",
...
export async function getAuditSummary(hours = 24): Promise<{
  ...
  const byAction = filtered.reduce(
    ...
    },
    {
      ...
      LAUDO_VALIDATED: 0,
      LAUDO_PUBLISHED_TO_PATIENT: 0,
      ANAMNESE_VALIDATED_BY_PROFESSIONAL: 0,
      PDF_VIEWED_BY_PATIENT: 0,
      PLANO_VALIDATED: 0,

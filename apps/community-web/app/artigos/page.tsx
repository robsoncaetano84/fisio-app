...
export const metadata = {
  title: 'Artigos compartilhados',
  description:
    'Artigos e materiais cientificos compartilhados na comunidade profissional SYNAP.',
};

export default async function ArticlesPage() {
  const resources = await getCommunityResources({ kind: 'article' });
        ...
        <section className="min-w-0">
          <div className="rounded-synap border border-synap-border bg-white p-6 shadow-subtle">
            <div className="flex items-start gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-synap bg-synap-secondary/10 text-synap-secondary">
                <FileText className="h-5 w-5" />
              </div>
              <div>
                <Badge tone="secondary">Evidencias cientificas</Badge>
                ...
                <p className="mt-2 max-w-3xl text-sm leading-6 text-synap-muted">
                  Espaco para organizar artigos, guias e materiais tecnicos com
                  foco em aplicabilidade clinica e discussao profissional.
                </p>
              </div>
            </div>
          </div>
        ...
        <aside className="space-y-4">
          <section className="rounded-synap border border-synap-border bg-white p-5 shadow-subtle">
            ...
            <p className="mt-3 text-sm leading-6 text-synap-muted">
              Artigos devem ser compartilhados com contexto, limites de
              aplicacao e relevancia para a pratica profissional.
            </p>

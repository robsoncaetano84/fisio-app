  ...
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_clinical_audit_logs_patient_created"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_clinical_audit_logs_action_created"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_clinical_audit_logs_actor_created"`,
    );
    await queryRunner.query(`DROP TABLE IF EXISTS "clinical_audit_logs"`);

    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_consent_purpose_logs_purpose_created"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_consent_purpose_logs_user_created"`,
    );
    await queryRunner.query(`DROP TABLE IF EXISTS "consent_purpose_logs"`);

    await queryRunner.query(
      `DROP INDEX IF EXISTS "idx_clinical_protocol_versions_created"`,
    );
    await queryRunner.query(
